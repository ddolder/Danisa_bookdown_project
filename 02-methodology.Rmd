...
The available data were from April to November. These data were divided by station and filtered by year and month. The data from the same month was averaged. The table 3.2 shows the processed data for the station 4917500. 
...
As it can be seen in table 3.1, there were not missing values in the station 4917450. The data was properly formatted and classified. In the following table, we can see the parameters statistics:
...
ggplot(data=data450, aes(Chla)) + 
  geom_histogram(binwidth=65, colour="black", fill="white", boundary = 65)+
